...
export default function TradingChart() {
  ...
  const cahrs = [
    ...
    const _graphData = async() => {
      ...
      let from = Math.floor(new Date().getTime() ) - (86400000*3)
    ...
    <>
      <div className="main-chart mb15 border">
        <div className="bg_pair_heading" >
          ...
          {show &&
            <h1><span className="stats_head">{pair.split('/')[0].toUpperCase()} 24h Volume: </span>{graphUpperData.v[0].toFixed(4)}</h1>
          }
        ...
        <ChartComponent 
          title=' '
          primaryXAxis={{valueType:"DateTime", minimum:new Date(Date.now() - 72 * 3600 * 1000), 
